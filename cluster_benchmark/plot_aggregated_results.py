...
def style_point_labels(
    ...
    for ct in LEGEND_ORDER:
        ...
        for s in sorted(by_circuit[ct], key=x_func):
            ...
            err = (s.errors + 0.5) / (s.shots * s.json_metadata["r"]) if s.shots and s.json_metadata.get("r") else 1e-10
            ordered_colors_and_xy.append((color, x_val, err))

    # Annotations are in creation order (ax.texts = order added)
    point_label_texts = []
    label_xy = []
    texts = [t for t in ax.texts if POINT_LABEL_PATTERN.match((t.get_text() or "").strip())]
...
def main():
    ...
    plot_opts = dict(
        ...
        plot_args_func=lambda k, g, _: {"color": g["color"]} if isinstance(g, dict) and "color" in g else {},
    )
