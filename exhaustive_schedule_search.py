...
def exhaustive_search(d=7, rounds=1, p_cnot=1e-3, method='ilp', use_custom_error_model=False, verbose=True):
    """
    ...
    colorcode_init = ColorCode(d=d, rounds=rounds, cnot_schedule="tri_optimal", p_cnot=init_p_cnot, exclude_non_essential_pauli_detectors=True)
    ...
    for idx, (r_idx, g_idx, b_idx) in enumerate(product(range(len(red_bulk)), 
                                                         range(len(green_bulk)), 
                                                         range(len(blue_bulk)))):
        ...
        colorcode = ColorCode(d=d, rounds=rounds, cnot_schedule=cnot_dict, p_cnot=build_p_cnot, exclude_non_essential_pauli_detectors=True)
        ...
        if distance > best_distance:
            ...
            if verbose:
                ...
                print(f"  R: {red_bulk[r_idx]}, G: {green_bulk[g_idx]}, B: {blue_bulk[b_idx]}")
        ...
        if verbose and (idx + 1) % 100 == 0:
            ...
            print(f"[{idx+1}/{total}] Elapsed: {elapsed:.1f}s, ETA: {remaining:.1f}s, Best: {best_distance}")
    ...
    for dist in unique_distances:
        ...
        print(f"Distance {dist}: {count} configurations ({100*count/len(distances):.1f}%)")
    
    return best_configs, all_results


def test_configs_on_d(configs, d, rounds=1, p_cnot=1e-3, method='ilp', use_custom_error_model=False, verbose=True):
    """
    ...
    for i, cfg in enumerate(configs):
        if verbose:
            print(f"[{i+1}/{len(configs)}] Testing config r={cfg['r_idx']}, g={cfg['g_idx']}, b={cfg['b_idx']} on d={d}...")
...
def visualize_config_minimal_error(cfg, d, rounds=1, p_cnot=1e-3, use_custom_error_model=False, save_prefix="minimal_error"):
    """
    ...
    import urllib.parse
    crumble_url = "https://algassert.com/crumble#circuit=" + urllib.parse.quote(str(colorcode.circuit), safe='')
    print(f"\nCrumble URL:\n{crumble_url}")
    ...
    title = f"d={d}, R:{cfg['r_schedule']}, G:{cfg['g_schedule']}, B:{cfg['b_schedule']}"
    visualize_errors_grid(colorcode, errors, title_prefix=title, save_name=f"{save_prefix}_d{d}.png")
...
def main():
    ...
    use_custom_error_model = False  # Use DEPOLARIZE1 before/after CNOT (with edge handling)
    # ============================================================
    ...
    if os.path.exists(d1_results_file):
        ...
        print("-" * 40)
        best_configs_d1, all_results_d1 = exhaustive_search(d=d1, rounds=rounds, p_cnot=p_cnot, method=method_d1, use_custom_error_model=use_custom_error_model, verbose=True)
        
        # Save d1 results
        save_results(all_results_d1, d1_results_file, d=d1, 
                     metadata={'rounds': rounds, 'p_cnot': p_cnot, 'method': method_d1, 'custom_error_model': use_custom_error_model})
    
    # Get configs with top N distance tiers
    best_configs = get_best_configs(all_results_d1, distance_key='distance', top_n=top_n_tiers)
    ...
    results_d2 = test_configs_on_d(best_configs, d=d2, rounds=rounds, p_cnot=p_cnot, method=method_d2, use_custom_error_model=use_custom_error_model, verbose=True)
    
    # Save combined results
    save_results(results_d2, f'best_configs_d{d1}_tested_on_d{d2}.pkl', 
                 metadata={'source_d': d1, 'tested_d': d2, 'rounds': rounds, 'p_cnot': p_cnot, 'custom_error_model': use_custom_error_model})
    ...
    results_d2_sorted = sorted(results_d2, key=lambda x: x.get(distance_key_d2, 0), reverse=True)
    ...
    for i, cfg in enumerate(results_d2_sorted[:10], 1):
        ...
        print(f"   d={d1} distance: {cfg['distance']}, d={d2} distance: {cfg.get(distance_key_d2, 'N/A')}")
