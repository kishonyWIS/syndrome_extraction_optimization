...
def setup_collision_detection(d=7):
    ...
    z_anc_to_data = {anc['qid']: get_data_qubits_canonical_order(anc['qid']) for anc in z_ancillas}
    bulk_anc_list = [anc['qid'] for anc in z_ancillas if anc['boundary'] is None]
    
    return z_anc_info, z_anc_to_data, bulk_anc_list


def count_collisions(r_schedule, g_schedule, b_schedule, z_anc_info, z_anc_to_data, bulk_anc_list):
    """
    ...
    return sum(
        1 for ts_dict in collisions.values() 
        for dqs in ts_dict.values() 
        if len(dqs) > 1
    )
...
def generate_zero_collision_schedules(results_file='search_results_d7.pkl', d=7):
    ...
    print(f"Loaded {len(optimal_configs)} optimal configurations with distance {max_dist}")
            ...
            for mod_g in g_vars:
                for mod_b in b_vars:
                    num_collisions = count_collisions(
                        mod_r, mod_g, mod_b, 
                        z_anc_info, z_anc_to_data, bulk_anc_list
                    )
...
def save_to_csv(schedules, output_file='zero_collision_schedules.csv'):
    ...
    fieldnames = [
        'index',
        'r_schedule', 'g_schedule', 'b_schedule',
        'orig_r_schedule', 'orig_g_schedule', 'orig_b_schedule',
        'orig_config_idx', 'circuit_distance', 'collisions'
    ]
...
def main():
    ...
    schedules = generate_zero_collision_schedules(
        results_file='search_results_d7.pkl',
        d=7
    )
    ...
    print(f"Circuit-level distance: {schedules[0]['circuit_distance'] if schedules else 'N/A'}")
    print(f"Output file: {output_file}")
    
    # Print first few schedules as examples
    print("\nFirst 5 schedules:")
    for i, sched in enumerate(schedules[:5]):
        print(f"  {i+1}. R={sched['r_schedule']}, G={sched['g_schedule']}, B={sched['b_schedule']}")
